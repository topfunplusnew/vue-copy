...
import { getAllBlogList, getBlogPost, blogSocialFilters, myblogedit, blogPost, Postimage, comments, getOsBlogList } from '@/services/api';
...
export const useBlogStore = defineStore('blog', () => {
  ...
  function getBlogList(refresh: boolean = false) {
    return new Promise((resovle, reject) => {
      ...
      getAllBlogList(blogs.value.args)
        .then((res) => {
          const data = res.data;
          if (!refresh && blogs.value?.items.length) data.items = [...(blogs.value?.items || []), ...data.items];
          blogs.value = data;
          resovle(res);
        })
        .catch((e) => reject(e))
        .finally(() => (blogs.value.loading = false));
    });
  }
  ...
  function getBlogosList(refresh: boolean = false) {
    return new Promise((resovle, reject) => {
      ...
      getOsBlogList(blogos.value.args)
        .then((res) => {
          const data = res.data;
          if (!refresh && blogos.value?.items.length) data.items = [...(blogos.value?.items || []), ...data.items];
          blogos.value = data;
          resovle(res);
        })
        .catch((e) => reject(e))
        .finally(() => (blogos.value.loading = false));
    });
  }
